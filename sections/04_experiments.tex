...
\begin{table}[htb]
  ...
  \hline
  \textbf{Dataset} & \textbf{Agent} & \textbf{Env.} & \textbf{Camera(s) used} & \textbf{Sessions} & \textbf{Multi-session GT} \\
  \hline
  KITTI & car & outdoor & \texttt{image\_2} (1 of 4) & 11 (7 eval.) & No \\
  HGE (HoloLens) & handheld/head-worn AR & indoor + outdoor & \texttt{hetlf} (1 of 4) & 5 & Yes \\
  HGE (iOS) & handheld AR & indoor + outdoor & \texttt{phone} (1) & 5 & Yes \\
  HYDRO & legged robot & indoor & \texttt{frontleft} (1 of 4) & 5 & Yes \\
  RobotCar & car & outdoor & \texttt{stereo\_centre} (1) & 4 (2 splits) & Yes \\
  VBR & car & outdoor & \texttt{camera\_left} (1 of 2) & 8 & No \\
  EuRoC & aerial vehicle (MAV) & indoor & \texttt{cam0} (1 of 2) & 11 (3 rooms) & Partial (per room) \\
  \hline
  \end{tabular*}
  \caption[Dataset overview]{The six evaluation datasets (HGE counted once per capture device),
    the agent and environment each was captured in, the single camera Polygraph's retrieval and
    pose estimation actually run on in the main experiments (parenthetical: how many are
    available in total, used by \mysecref{sec:experiments-ablation-cameras}'s camera-pairing
    ablation), how many sessions exist, and whether ground truth places more than one session in
    one shared coordinate frame at all. ``Partial'' for EuRoC: shared within each of the three
    rooms, not across rooms (\mysecref{sec:experiments-euroc}).}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.49\textwidth]{images/04_experiments/01_datasets/dataset_hge_hololens.png}
  \hfill
  \includegraphics[width=0.49\textwidth]{images/04_experiments/01_datasets/dataset_hge_ios.png}
