...
\begin{table}[htb]
  ...
  \caption[KITTI ATE, ORB-SLAM3 backbone]{KITTI ATE (m), ORB-SLAM3 backbone. ``Raw (ours)'' is
    ...
    all 7 sequences, Mean the arithmetic mean of the same 7. Bold marks the best result in a row,
...
\begin{table}[htb]
  ...
  \hline
  Total (n=7) & 28.80 & 19.07 & 40.06 & 34.96 & \textbf{12.56} & \underline{13.58} & \underline{13.58} \\
  Mean (n=7) & 9.09 & 5.85 & 14.32 & 12.85 & \textbf{3.50} & 3.80 & \underline{3.79} \\
  \hline
  \end{tabular*}
...
the same way the ATE numbers themselves are (Umeyama, no scale correction). Every sequence is
rotated in-plane to its own principal axis before plotting, so a near-straight sequence like 06
does not force a page-filling, mostly-empty panel just to preserve true-north orientation; only
...
(\mysecref{sec:experiments-kitti}), the single- and multi-session columns are visually
identical for every other sequence, by construction, not a plotting error. Where a real,
if small, improvement does exist, the multi-session panel's inset zooms into the single point
along the trajectory where the single- and multi-session estimates diverge most, so that
improvement stays visible even at full-trajectory scale.

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.82\textwidth]{images/04_experiments/04_ablation/kitti_ablation_grid_orb.png}
  \caption[KITTI trajectory grid, ORB-SLAM3 backbone]{All 7 loop-closure KITTI sequences, ORB-SLAM3
    backbone: raw odometry, single-session, and multi-session output (green), against GT (red),
    GT-aligned. Inset on the multi-session column zooms into the point of largest single-vs-multi
    divergence along that sequence.}
  \label{fig:experiments-ablation-kitti-orb}
\end{figure}

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.82\textwidth]{images/04_experiments/04_ablation/kitti_ablation_grid_vins.png}
  \caption[KITTI trajectory grid, VINS-Fusion backbone]{Same layout as
    \myfigref{fig:experiments-ablation-kitti-orb}, VINS-Fusion backbone. Sequence 08's raw
    odometry visibly drifts from GT and stays uncorrected in both the single- and
    multi-session columns, the same drift pattern in all three panels
    (\mytabref{tab:experiments-kitti-vins}'s detection-miss caveat: zero intra- or
    cross-session edges found for this sequence on this backbone).}
  \label{fig:experiments-ablation-kitti-vins}
\end{figure}
