...
for every other sequence, by construction, not a plotting error. Each backbone's grid is one
continuous image, cropped at exactly the page boundary and carried onto the next page with no
gap or caption in between, so it reads as one uninterrupted grid rather than two separate
figures.

\clearpage
\noindent\includegraphics[width=\textwidth]{images/04_experiments/04_ablation/kitti_grid_orb_part1.png}

\clearpage
\noindent\includegraphics[width=\textwidth]{images/04_experiments/04_ablation/kitti_grid_orb_part2.png}
\captionof{figure}[KITTI trajectory grid, ORB-SLAM3 backbone]{KITTI trajectory grid, ORB-SLAM3
  backbone, all 7 loop-closure sequences: raw odometry, single-session, and multi-session output
  (green) against GT (red), GT-aligned.}
\label{fig:experiments-ablation-kitti-orb}

\clearpage
\noindent\includegraphics[width=\textwidth]{images/04_experiments/04_ablation/kitti_grid_vins_part1.png}

\clearpage
\noindent\includegraphics[width=\textwidth]{images/04_experiments/04_ablation/kitti_grid_vins_part2.png}
\captionof{figure}[KITTI trajectory grid, VINS-Fusion backbone]{Same layout as
  \myfigref{fig:experiments-ablation-kitti-orb}, VINS-Fusion backbone. Sequence 08's raw
  odometry visibly drifts from GT and stays uncorrected in both the single- and multi-session
  columns, the same drift pattern in all three panels (\mytabref{tab:experiments-kitti-vins}'s
  detection-miss caveat: zero intra- or cross-session edges found for this sequence on this
  backbone).}
